...
| REQ_014 | 2026-07-15 | 使用者口述（out-of-band，REQ_013 之延伸） | P1 | `/admin` 頁面改為帳號密碼登入（bcrypt 密碼雜湊 + session cookie），取代原本的 API Key 輸入；未登入時顯示登入卡片，登入成功後才顯示排程狀態內容；既有 admin scope API Key 呼叫 `/api/v1/admin/*` 之方式仍相容不受影響 | `02_system_design/outputs/api_spec.md` | `app/auth.py`(`hash_password`/`require_admin_access`), `app/routers/auth.py`(新檔), `app/db_models.py`(`AdminUser`), `app/main.py`(`SessionMiddleware`), `app/static/admin.{html,js}` | `tests/test_api_auth.py`, `tests/test_admin_jobs.py` | ✅ 已驗證 |
| REQ_015 | 2026-07-16 | 使用者口述（out-of-band，REQ_014 之延伸） | P2 | `/dashboard` 移除 API Key 輸入框，改為「登入」按鈕導向 `/admin` 共用登入（含安全的站內 `next` 導回機制）；查詢類端點（公司清單/搜尋/財務/股價/預測/回測）改為公開瀏覽不需任何憑證，新增公司（寫入）仍需登入或 admin scope API Key；主動放寬構面 1 存取控制之風險接受理由詳見 `security_requirements.md` §五之三 | `02_system_design/outputs/api_spec.md`、`02_system_design/outputs/threat_model.md` §三之三 | `app/auth.py`(`optional_read_access`), `app/routers/companies.py`, `app/routers/predictions.py`, `app/static/dashboard.{html,js}`, `app/static/admin.js`(`safeNextPath`), `app/db_session.py`(`pool_pre_ping`) | `tests/test_auth.py`, `tests/test_api_companies.py`, `tests/test_api_predictions.py` | ✅ 已驗證 |
| REQ_016 | 2026-07-17 | 使用者口述（out-of-band，補完 REQ_004/005/006 之 weekly_predict 斷鏈） | P1 | `weekly_predict` 排程由 stub 接上真實邏輯：新增財報 YoY/QoQ 特徵工程模組，財報因子模型改採跨公司彙總訓練；因真實財報樣本量不足 10 筆訓練門檻，優雅降級為僅時間序列模型出預測（`model_version` 加註 `-ts-only`，不偽裝成雙模型融合），待樣本數累積足夠後會自動切換為真正融合，無需再次介入 | 無獨立設計文件（沿用既有 `db_schema.sql`） | `app/prediction/features.py`（新檔）, `app/jobs.py`(`run_weekly_predict`), `app/main.py`, `app/static/dashboard.js`（KPI 標籤誠實反映降級狀態） | `tests/test_prediction_features.py`（新檔，10 項）, `tests/test_jobs.py`（新增 6 項） | ✅ 已驗證 |
