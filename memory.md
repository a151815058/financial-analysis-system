...
### 2026-07-17 — Session 2（續）：/dashboard 登入後新增「後台管理」導向按鈕

- 使用者要求登入後能從 `/dashboard` 導到 `/admin`，符合使用者體驗（`/admin` 原本就有「回財務儀表板」
  連結，`/dashboard` 缺對應的反向導覽）。在登入狀態列新增「後台管理」按鈕，僅登入後顯示。
- 用 Puppeteer 驅動系統既有 Chrome（`npm install puppeteer-core --no-save`，比照 REQ_010 先例不下載
  額外 Chromium）實際登入後截圖/檢查：未登入時按鈕隱藏、登入後正確顯示且點擊後真的導到 `/admin`，
  驗證完後即移除暫時安裝的 `node_modules`/驗證腳本，不留在版控。
- 純前端改動，未新增自動化測試（UI 顯示邏輯以真實瀏覽器互動驗證），130 項既有測試無回歸。
